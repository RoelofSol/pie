...
fun StoreReadTxn.directlyAffectedTaskKeys(changedFiles: Collection<PPath>, logger: Logger): HashSet<TaskKey> {
  ...
  val isAffected = { key : TaskKey ->
    if( observability(key) == Observability.Detached ) { logger.trace( "  * Is not attached ") }
  ...
  for(changedFile in changedFiles) {
    ...
    for(key in requirees) {
      ...
      if( observability(key) == Observability.Detached ) { logger.trace( "  * Is Detached ") }
    ...
    if(generator != null) {
      ...
      if( observability(generator) == Observability.Detached ) { logger.trace( "  * Is Detached ") }
